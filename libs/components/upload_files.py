...
class DocumentProcessor:
    ...
    def process_documents(
        ...
        if st.session_state["uploaded_docs"]:
            ...
            st.sidebar.caption("NOTE: Press on the process button to be able to chat with your documents.")
            if st.sidebar.button("Process docs", key="process_btn", type="primary", use_container_width=True):
                self._process_uploaded_docs()
        else:
            if st.session_state.show_warning:
                self.file_upload_container.warning(
                    "There is no document uploaded. "
                    "If you have already processed files before ignore this message.", icon="⚠️"
                    )

    def _process_uploaded_docs(
            self
            ):
    ...
    def _cleanup_if_all_files_processed(
            self,
            processed_files: List[Path]
            ):
