...
def set_api_keys():
    ...
    api_keys_container.checkbox(
        "Enable Langchain Tracing", value=False, key="enable_langchain_tracing"
    )
    os.environ[
        "LANGCHAIN_TRACING_V2"
    ] = f"{str(st.session_state.enable_langchain_tracing).lower()}"
...
def set_specific_api_key(key_name):
    api_keys_container = st.sidebar.container(border=True)
