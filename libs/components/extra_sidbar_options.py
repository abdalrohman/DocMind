...
class HistoryExporter:
    ...
    def _export(self, file_path, data, format_func=str):
        with open(file_path, "w") as f:
            f.write(format_func(data))

    def export_history(self, format, mime_type):
        file_path = (
            self.export_history_dir
            / f"chat_history-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.{format}"
        )
...
def additional_options(history: StreamlitChatMessageHistory):
